// ============================================================================
// FLOCK DETECTOR v6.0-ADV — M5Stack Cardputer ADV Port
// ============================================================================
// Expansion Hat GPS Routing (Pins 13/15), Median Battery, 3D Radar Particles
// ============================================================================
...
#define BG_COLOR      lgfx::color565( 10,  20,  48)  
#define CARD_COLOR    lgfx::color565( 18,  36,  80)  
#define CARD_BORDER   lgfx::color565( 26,  42,  58)  
#define PANEL_BG      lgfx::color565( 10,  20,  48)
#define HEADER_COLOR  lgfx::color565(  0, 238, 255)  
#define TEXT_COLOR    lgfx::color565(255, 255, 255)  
#define DIM_COLOR     lgfx::color565(  0, 170, 204)  
#define DIM2_COLOR    lgfx::color565( 26,  42,  58)  
#define ACCENT_COLOR  lgfx::color565( 50,  255, 100) 
#define TEAL_COLOR    lgfx::color565(  0, 180, 180)  
#define CYAN2_COLOR   lgfx::color565(  0, 255, 255)  
#define ALERT_COLOR   lgfx::color565(255,  50,  50)  
#define TOAST_COLOR   lgfx::color565(255, 224,   0)  
#define WARN_COLOR    lgfx::color565(255, 165,   0)  
#define GRID_COLOR    lgfx::color565( 26,  42,  58)  
#define HILIGHT_BG    lgfx::color565( 18,  36,  80)  

// ============================================================================
// CONFIGURATION (V6.0 LoRa GNSS Hat Pinout)
// ============================================================================
#define GPS_RX_PIN   15   // G15 = ESP receives from GPS (GPS TX → ESP RX)
#define GPS_TX_PIN   13   // G13 = ESP transmits to GPS  (ESP TX → GPS RX)  
...
// CONFIDENCE SCORING
#define CONF_MAC_TIER1        55  
#define CONF_MAC_TIER2        35
...
// ============================================================================
// BATTERY ENGINE (MEDIAN + DIODE CALIBRATION)
// ============================================================================
...
int32_t get_median_voltage() {
    ...
    if (sorted[BATTERY_SAMPLE_SIZE / 2] == 0) {
...
uint8_t get_unified_battery_pct(int16_t mv) {
    ...
    if (mv > VOLT_HIGH) {
        return map(mv, VOLT_HIGH, VOLT_MAX, 80, 100);
    } 
    else if (mv > VOLT_NOMINAL) {
        return map(mv, VOLT_NOMINAL, VOLT_HIGH, 50, 80);
    } 
    else if (mv > VOLT_LOW) {
        return map(mv, VOLT_LOW, VOLT_NOMINAL, 20, 50);
    } 
    else {
        return map(mv, VOLT_MIN, VOLT_LOW, 0, 20);
    }
}
...
bool is_device_charging(int32_t current_mv) {
    ...
    if (millis() - last_check > 2500) {
        if (current_mv - baseline_mv >= 15) { 
            chg_state = true; 
            baseline_mv = current_mv; 
        }
        else if (baseline_mv - current_mv >= 15) { 
            chg_state = false; 
            baseline_mv = current_mv; 
        }
        else { 
        ...
        if (current_mv < 3800 && !chg_state && (baseline_mv - current_mv > 0)) {
            chg_state = false; 
        }
...
void dedicated_charging_loop() {
    ...
    while (true) {
        ...
        if (fill_w > 0) {
            spr.fillRect(bar_x + 1, bar_y + 1, fill_w, bar_h - 2, ACCENT_COLOR);
        }
        
        spr.setTextColor(DIM_COLOR, BG_COLOR); 
        spr.setTextSize(1); 
        spr.setCursor(35, 115); 
        spr.print("PRESS ANY KEY TO BOOT DEVICE"); 
...
int check_mac_prefix_tiered(const uint8_t* mac) {
    ...
    for (int i = 0; i < NUM_MAC_TIER1; i++) {
        if (strncasecmp(mac_str, mac_prefixes_tier1[i], 8) == 0) return CONF_MAC_TIER1;
    }
    for (int i = 0; i < NUM_MAC_TIER2; i++) {
        if (strncasecmp(mac_str, mac_prefixes_tier2[i], 8) == 0) return CONF_MAC_TIER2;
    }
    return 0;
}
...
bool is_mac_recently_seen(const String& mac) {
    ...
    for (int i = 0; i < MAX_SEEN_MACS; i++) {
        if (seen_macs[i] == mac) {
            if ((now - seen_mac_timestamps[i]) < REDETECT_WINDOW_MS) {
                return true;
            } else { 
                seen_mac_timestamps[i] = now; 
                return false; 
            }
        }
    }
...
// ============================================================================
// THREAT EQUALIZER DATA (3D Hologram Radar Base)
// ============================================================================
#define radar_cx 62
#define radar_cy 74
#define radar_r  50
...
void add_blip(uint16_t blip_color) {
    ...
    if(radial_spikes[prev] < 0.6f) { 
        radial_spikes[prev] = 0.6f; 
        radial_colors[prev] = blip_color; 
    }
    
    if(radial_spikes[next] < 0.6f) { 
        radial_spikes[next] = 0.6f; 
        radial_colors[next] = blip_color; 
    }
}
...
void trigger_toast(const String& type, const String& name, int confidence) {
    ...
    if (toast_text.length() > 14) {
        toast_text = toast_text.substring(0, 14);
    }
...
void add_to_capture_history(const char* type, const char* mac, const String& name, int rssi, int confidence) {
    for (int i = CAPTURE_HISTORY_SIZE - 1; i > 0; i--) {
    ...
    if (capture_history_count < CAPTURE_HISTORY_SIZE) {
        capture_history_count++;
    }
}

void log_detection(const char* type, const char* proto, int rssi, const char* mac, const String& name, int channel, int tx_power, const String& extra_data, const char* detection_method, int confidence) {
    ...
    if (is_new) {
        ...
        if (strcmp(proto, "WIFI") == 0) { 
            ...
            lifetime_wifi++; 
            session_flock_wifi++; 
            blip_col = HEADER_COLOR; 
        } else { 
            session_ble++; 
            lifetime_ble++; 
            blip_col = TFT_MAGENTA; 
        }
        
        if (strstr(type, "RAVEN") != NULL) { 
            session_raven++; 
            blip_col = TEAL_COLOR; 
        } else if (strcmp(proto, "BLE") == 0) { 
            session_flock_ble++; 
        }
    ...
    last_cap_time = current_time_formatted; 
    ...
    if (millis() - last_log_update > 500) {
        ...
        sprintf(log_buf, "%s | %ddBm | %s", cleanName.c_str(), rssi, short_mac(mac_str).c_str());
        bool is_duplicate = false;
        
        for (int i = 0; i < 7; i++) { 
            if (live_logs[i].indexOf(cleanName) >= 0 && cleanName != "WIFI" && cleanName != "BLE") { 
                is_duplicate = true; 
                break; 
            } 
        }
        ...
        if (!is_duplicate) {
            ...
            live_logs[0] = "!" + String(confidence) + "%|" + String(log_buf); 
    ...
    if (is_new && sd_available) {
        File file = SD.open(current_log_file.c_str(), FILE_APPEND);
        if (file) {
            file.printf("%lu,%s,%d,%s,%s,%d,%s,\"%s\",%d,\"%s\",%d,%s\n", 
                now_ms, type, channel, proto, rssi, mac, name.c_str(), tx_power, detection_method, confidence, extra_data.c_str());
            file.close();
        }
    }
}
...
void locator_add_sample(const String& mac, int rssi) {
    if (!locator_active || mac != locator_target_mac) return;
    ...
    if (idx >= LOC_MAX_SAMPLES) {
        ...
        for (int i = 1; i < LOC_MAX_SAMPLES; i++) {
            if (locator_samples[i].rssi < locator_samples[weakest].rssi) {
                weakest = i;
            }
        }
        if (rssi > locator_samples[weakest].rssi) {
            idx = weakest; 
        } else { 
            xSemaphoreGive(dataMutex); 
            return; 
    ...
    if (locator_sample_count >= LOC_MIN_SAMPLES_EST) {
        ...
        if (sum_w > 0) {
            locator_est_lat = sum_wlat / sum_w; 
            locator_est_lng = sum_wlng / sum_w; 
...
void locator_start(const String& mac, const String& name) {
    xSemaphoreTake(dataMutex, portMAX_DELAY);
    locator_active = true; 
    locator_target_mac = mac; 
    locator_target_name = name;
    locator_sample_count = 0; 
    locator_has_estimate = false; 
    locator_peak_rssi = -120;
    locator_est_distance = 0; 
    locator_bearing = 0; 
    locator_confidence_radius = 0;
...
void wifi_sniffer_packet_handler(void* buff, wifi_promiscuous_pkt_type_t type) {
    ...
    if (mac_score > 0) { 
        ...
        methods += (mac_score == CONF_MAC_TIER1) ? "mac_t1 " : "mac_t2 "; 
    }
    ...
    if (confidence >= CONFIDENCE_ALARM_THRESHOLD) {
        ...
        locator_add_sample(String(mac_str), ppkt->rx_ctrl.rssi);
        methods.trim();
        log_detection("FLOCK_WIFI", "WIFI", ppkt->rx_ctrl.rssi, mac_str, name_str, ppkt->rx_ctrl.channel, 0, frame_type_str, methods.c_str(), confidence);
        ...
        if (millis() - last_log_update > LOG_UPDATE_DELAY) {
            xSemaphoreTake(dataMutex, portMAX_DELAY);
            String cleanName = (name_str != "Hidden" && name_str != "") ? name_str : "WiFi";
            if (cleanName.length() > 9) cleanName = cleanName.substring(0, 9);
            
            bool is_duplicate = false;
            for (int i = 0; i < 7; i++) { 
                if (live_logs[i].indexOf(cleanName) >= 0 && cleanName != "WiFi") { 
                    is_duplicate = true; 
                    break; 
                } 
            }
            ...
            if (!is_duplicate) {
                char log_buf[64]; 
                sprintf(log_buf, "%s | %ddBm | %s", cleanName.c_str(), ppkt->rx_ctrl.rssi, short_mac(String(mac_str)).c_str());
                for (int i = 6; i > 0; i--) live_logs[i] = live_logs[i - 1];
                live_logs[0] = String(log_buf); 
                last_log_update = millis();
            }
            xSemaphoreGive(dataMutex);
        }
...
class AdvertisedDeviceCallbacks : public NimBLEAdvertisedDeviceCallbacks {
    void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
        ...
        if (confidence >= CONFIDENCE_ALARM_THRESHOLD) {
            ...
            methods.trim();
            log_detection(capture_type.c_str(), "BLE", advertisedDevice->getRSSI(), addr.toString().c_str(), dev_name, 0, tx_power, extra_data, methods.c_str(), confidence);
            ...
            if (millis() - last_log_update > LOG_UPDATE_DELAY) {
                ...
                bool is_duplicate = false;
                for (int i = 0; i < 7; i++) { 
                    if (live_logs[i].indexOf(cleanName) >= 0 && cleanName != "BLE") { 
                        is_duplicate = true; 
                        break; 
                    } 
                }
                ...
                if (!is_duplicate) {
                    char log_buf[64]; 
                    sprintf(log_buf, "%s | %ddBm | %s", cleanName.c_str(), advertisedDevice->getRSSI(), short_mac(mac_string).c_str());
                    for (int i = 6; i > 0; i--) live_logs[i] = live_logs[i - 1];
                    live_logs[0] = String(log_buf); 
                    last_log_update = millis();
                }
...
void draw_header_spr(int screen_num) {
    ...
    int32_t med_mv = get_median_voltage();
    ...
    if (abs(raw_bat - display_bat) >= 2 || raw_bat == 100 || raw_bat == 0) { 
        display_bat = raw_bat; 
    }
    ...
    if (chg) {
        spr.drawLine(DISP_W - 65, 4, DISP_W - 67, 8, TEXT_COLOR);
        spr.drawLine(DISP_W - 67, 8, DISP_W - 63, 8, TEXT_COLOR);
        spr.drawLine(DISP_W - 64, 8, DISP_W - 66, 12, TEXT_COLOR);
        spr.setTextColor(ACCENT_COLOR, BG_COLOR);
    } else {
...
void draw_toast_spr() {
    ...
    if (millis() - toast_start > TOAST_DURATION_MS) { 
        toast_active = false; 
        return; 
    }
    ...
    spr.print("! "); 
    spr.print(toast_text);
}

void drawCard(int x, int y, int w, int h) {
    spr.fillRect(x, y, w, h, CARD_COLOR);
    spr.drawRect(x, y, w, h, CARD_BORDER); 
}
...
void draw_scanner_screen() {
    spr.fillRect(116, 19, DISP_W - 116, DISP_H - 19, BG_COLOR);
    spr.fillRect(0, 19, 116, DISP_H - 19, BG_COLOR); 
    
    draw_header_spr(0);
    
    spr.setClipRect(0, 19, 116, DISP_H - 19);
    float TILT = 0.85f; 

    spr.drawEllipse(radar_cx, radar_cy + 6, radar_r, radar_r * TILT, lgfx::color565(5, 10, 24));
    ...
    for(int i = 0; i < NUM_PARTICLES; i++) {
        if (noise_life[i] <= 0) {
            ...
            noise_max_life[i] = random(200, 400); 
            noise_life[i] = noise_max_life[i];
        } else {
            noise_life[i]--; 
            float life_ratio = (float)noise_life[i] / (float)noise_max_life[i]; 
            float peak = 0.5f; 
            int intensity;
            
            if (life_ratio > peak) {
                intensity = (int)(255.0f * (1.0f - life_ratio) / (1.0f - peak));
            } else {
                intensity = (int)(255.0f * (life_ratio / peak));
            }
            ...
            uint16_t p_col = (i % 3 == 0) ? lgfx::color565(intensity, intensity, intensity) : lgfx::color565(0, intensity, intensity); 
            int px = radar_cx + noise_r[i] * cos(noise_a[i]); 
            int py = radar_cy + noise_r[i] * sin(noise_a[i]) * TILT;
    ...
    float breathe = sin(millis() / 500.0f); 
    int dynamic_r = radar_r + (int)(4 * breathe);
    float cos_rot = cos(hud_rotation); 
    float sin_rot = sin(hud_rotation);
    
    spr.drawLine(radar_cx - dynamic_r * cos_rot, radar_cy - dynamic_r * sin_rot * TILT, 
                 radar_cx + dynamic_r * cos_rot, radar_cy + dynamic_r * sin_rot * TILT, modulated_col);
                 
    spr.drawLine(radar_cx - dynamic_r * -sin_rot, radar_cy - dynamic_r * cos_rot * TILT, 
                 radar_cx + dynamic_r * -sin_rot, radar_cy + dynamic_r * cos_rot * TILT, modulated_col);

    float sweep_deg = (millis() / 1500.0f) * 6.28318f;
    for (int i = 1; i <= 24; i++) {
        float trail_angle = sweep_deg - (i * 0.05f); 
        ...
        uint16_t fade_col = lgfx::color565(tr, tg, tb);
    ...
    float angle_step = 6.28318f / NUM_RADIAL_BANDS; 
    ...
    spr.setCursor(px + 4, 25); 
    spr.print("WF:CH"); 
    spr.print(current_channel);
    
    spr.drawRect(px + 58, 22, 54, 14, ble_scanning ? TFT_MAGENTA : DIM2_COLOR);
    spr.setTextColor(ble_scanning ? TFT_MAGENTA : DIM_COLOR, BG_COLOR);
    spr.setCursor(px + 60, 25); 
    spr.print("BLE:SCN");

    spr.setTextColor(ACCENT_COLOR, BG_COLOR);
    spr.setTextSize(1);
    spr.setCursor(px + 2, 42); 
    spr.print("WIFI HITS");
    
    spr.setTextColor(TOAST_COLOR, BG_COLOR);
    spr.setTextSize(2);
    spr.setCursor(px + 2, 52); 
    spr.print(sw);

    spr.setTextColor(ACCENT_COLOR, BG_COLOR);
    spr.setTextSize(1);
    spr.setCursor(px + 2, 70); 
    spr.print("BLE HITS");
    
    spr.setTextColor(TFT_MAGENTA, BG_COLOR);
    spr.setTextSize(2);
    spr.setCursor(px + 2, 80); 
    spr.print(sb);
...
void draw_stats_screen() {
    xSemaphoreTake(dataMutex, portMAX_DELAY);
    long tw = session_flock_wifi;
    long tb = session_flock_ble;
    long tr = session_raven;
    xSemaphoreGive(dataMutex);
    ...
    spr.setCursor(10, 34); 
    spr.print("WIFI SESS");
    spr.setTextColor(TOAST_COLOR, CARD_COLOR); 
    spr.setTextSize(3); 
    spr.setCursor(10, 46); 
    spr.print(tw);
    ...
    spr.setCursor(88, 34); 
    spr.print("BLE SESS");
    spr.setTextColor(TFT_MAGENTA, CARD_COLOR); 
    spr.setTextSize(3);
    spr.setCursor(88, 46); 
    spr.print(tb);
    ...
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
    spr.setTextSize(1);
    spr.setCursor(166, 34); 
    spr.print("RAVEN");
    spr.setTextColor(TEAL_COLOR, CARD_COLOR); 
    spr.setTextSize(3); 
    spr.setCursor(166, 46); 
    spr.print(tr);

    drawCard(67, 95, 106, 32);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
    spr.setTextSize(1);
    spr.setCursor(85, 100); 
    spr.print("SESSION TIME");
    spr.setTextColor(TEAL_COLOR, CARD_COLOR); 
    spr.setTextSize(2);
    spr.setCursor(77, 110);
    spr.print(format_time((millis() - session_start_time) / 1000));
...
void draw_last_capture_screen() {
    xSemaphoreTake(dataMutex, portMAX_DELAY);
    String t_type = last_cap_type;
    String t_time = last_cap_time;
    String t_mac = last_cap_mac;
    String t_name = last_cap_name;
    ...
    if (t_type == "None") {
        ...
        spr.drawRect(4, 22, DISP_W - 8, 20, ccol);
        
        spr.setTextColor(ccol, HILIGHT_BG); 
        spr.setTextSize(1);
        spr.setCursor(8, 26); 
        spr.print(t_type);
        
        spr.setTextColor(TEXT_COLOR, HILIGHT_BG);
        spr.setCursor(DISP_W - 70, 26); 
        spr.print("@ "); 
        spr.print(t_time);
        ...
        spr.setTextColor(CYAN2_COLOR, BG_COLOR); 
        spr.setTextSize(2);
        spr.setCursor(4, 82); 
        spr.print(t_rssi); 
        spr.print("dBm");
        ...
        spr.setTextSize(2);
        spr.setCursor(4, 116); 
        spr.print(confidence_label(t_conf));
    }
    draw_toast_spr();
}
...
void draw_capture_history_screen() {
    ...
    for (int i = 0; i < local_count; i++) {
        local_history[i] = capture_history[i];
    }
    xSemaphoreGive(dataMutex);
    ...
    if (local_count == 0) {
        ...
        for (int i = 0; i < local_count && i < CAPTURE_HISTORY_SIZE; i++) {
            ...
            if (local_history[i].name != "" && local_history[i].name != "Hidden" && local_history[i].name != "Unknown") {
                ...
                nom = short_mac(local_history[i].mac); 
            }
            ...
            char cpct[5]; 
            sprintf(cpct, "%d%%", local_history[i].confidence);
            spr.setTextColor(rcol, (i % 2 == 0) ? HILIGHT_BG : BG_COLOR);
            spr.setCursor(DISP_W - 50, y + 6); 
            spr.print(cpct);
            
            spr.setTextColor(TEXT_COLOR, (i % 2 == 0) ? HILIGHT_BG : BG_COLOR);
            spr.setCursor(DISP_W - 35, y + 6); 
            spr.print(local_history[i].time);
...
void draw_live_log_screen() {
    xSemaphoreTake(dataMutex, portMAX_DELAY);
    String t_logs[7]; 
    for (int i = 0; i < 7; i++) t_logs[i] = live_logs[i];
    xSemaphoreGive(dataMutex);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        String clean_line = t_logs[i];
        if (is_hit) clean_line = clean_line.substring(1); 
        
        uint16_t row_bg = is_hit ? HILIGHT_BG : BG_COLOR;
        
        spr.fillRect(0, y, DISP_W, 15, row_bg);
        if (is_hit) spr.fillRect(0, y, 3, 15, TOAST_COLOR);
        
        spr.setTextColor(is_hit ? TOAST_COLOR : ACCENT_COLOR, row_bg); 
        spr.setTextSize(1);
        spr.setCursor(6, y + 4);
        
        if (clean_line.length() > 40) clean_line = clean_line.substring(0, 40);
...
void draw_gps_screen() {
    ...
    spr.setTextSize(1);
    spr.setCursor(10, 26); 
    spr.print("SATELLITES");
    spr.setTextColor(sats >= 4 ? TEXT_COLOR : WARN_COLOR, CARD_COLOR); 
    spr.setTextSize(3);
    spr.setCursor(10, 36); 
    spr.print(sats);
    ...
    if (has_loc && !stale) {
        ...
        spr.print("LOCKED");
        int r = (millis() / 80) % 15;
        spr.drawCircle(anim_cx, anim_cy, r, ACCENT_COLOR);
        spr.fillCircle(anim_cx, anim_cy, 3, ACCENT_COLOR);
    } else if (has_loc && stale) {
        spr.setTextColor(WARN_COLOR, CARD_COLOR); 
        spr.setTextSize(2);
        spr.setCursor(96, 36); 
        spr.print("LOST");
    ...
    if (has_loc && !stale) {
        ...
        spr.setCursor(8, 82); 
        spr.print(gps.location.lat(), 5);

        drawCard(120, 68, 116, 30);
        spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
        spr.setTextSize(1);
        spr.setCursor(124, 72); 
        spr.print("LONGITUDE");
        spr.setTextColor(HEADER_COLOR, CARD_COLOR);
        spr.setCursor(124, 82); 
        spr.print(gps.location.lng(), 5);
        ...
        spr.setCursor(8, 116);
        char spd[12]; 
        sprintf(spd, "%.1f mph", gps.speed.isValid() ? gps.speed.mph() : 0.0);
        spr.print(spd);

        drawCard(90, 102, 80, 28);
        spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
        spr.setTextSize(1);
        spr.setCursor(94, 106); 
        spr.print("HEADING");
        spr.setTextColor(TEAL_COLOR, CARD_COLOR);
        spr.setCursor(94, 116);
        char hdg[8]; 
        sprintf(hdg, "%d deg", gps.course.isValid() ? (int)gps.course.deg() : 0);
        spr.print(hdg);
        ...
        if (gps.altitude.isValid()) {
            ...
            spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
            spr.setTextSize(1);
            spr.setCursor(180, 106); 
            spr.print("ALT");
            spr.setTextColor(TEAL_COLOR, CARD_COLOR);
            spr.setCursor(180, 116);
            char alt[8]; 
            sprintf(alt, "%dm", (int)gps.altitude.meters());
            spr.print(alt);
        }
    } else {
        spr.setTextColor(ACCENT_COLOR, BG_COLOR); 
        ...
        spr.setTextColor(ACCENT_COLOR, BG_COLOR);
        spr.setCursor(4, 88);
        spr.print("VALID NMEA: "); 
        spr.setTextColor(TEXT_COLOR, BG_COLOR); 
        spr.print(passed);
        
        spr.setTextColor(ACCENT_COLOR, BG_COLOR);
        spr.setCursor(4, 100);
        spr.print("CHECKSUM FAILS: "); 
        spr.setTextColor(TEXT_COLOR, BG_COLOR); 
        spr.print(failed);
        ...
        if (chars < 10) {
            ...
            spr.setCursor(4, 116);
            spr.print("NO DATA: CHECK TX/RX PINS");
        } else if (passed == 0) {
            spr.setTextColor(WARN_COLOR, BG_COLOR);
            spr.setCursor(4, 116);
            spr.print("WAITING FOR SATELLITE LOCK");
        }
    }
...
void draw_chart_screen() {
    ...
    int chart_top = 28;
    int chart_h = DISP_H - chart_top - 16;
    int bar_w = DISP_W / CHART_BARS;

    spr.drawRect(0, chart_top, DISP_W, chart_h, HEADER_COLOR);
    ...
    for (int i = 0; i < CHART_BARS; i++) {
        ...
        if (bar_h > 0) {
            spr.fillRect(i * bar_w + 1, chart_top + chart_h - bar_h, bar_w - 1, bar_h, TOAST_COLOR);
        }
    }

    spr.setTextColor(ACCENT_COLOR, BG_COLOR); 
    spr.setTextSize(1);
    spr.setCursor(4, DISP_H - 12); 
    spr.print("25s ago");
    
    spr.setCursor(DISP_W - 30, DISP_H - 12); 
    spr.print("now");
    
    spr.setCursor(DISP_W / 2 - 20, DISP_H - 12); 
    spr.print("MAX: "); 
    spr.setTextColor(TEXT_COLOR, BG_COLOR); 
    spr.print(max_val);
}
...
void draw_proximity_screen() {
    ...
    String cap_name = last_cap_name;
    String cap_mac = last_cap_mac;
    xSemaphoreGive(dataMutex);
    ...
    if (cap_type == "None") {
        ...
        spr.setTextSize(1);
        spr.setCursor(10, 26); 
        spr.print("SIGNAL STRENGTH");
        
        spr.setTextColor(CYAN2_COLOR, CARD_COLOR); 
        spr.setTextSize(3);
        spr.setCursor(10, 36); 
        spr.print(rssi);
        spr.setTextSize(1); 
        ...
        spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
        spr.setTextSize(1);
        spr.setCursor(126, 26); 
        spr.print("CONFIDENCE");
        
        spr.setTextColor(ccol, CARD_COLOR); 
        spr.setTextSize(3);
        spr.setCursor(126, 36); 
        spr.print(conf);
        spr.setTextSize(1); 
        spr.print("%");
...
void draw_device_info_screen() {
    ...
    spr.setTextSize(1);
    spr.setCursor(10, 27); 
    spr.print("FLOCK DETECTOR v6.0 [ADV]"); 
    ...
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
    spr.setTextSize(1); 
    spr.setCursor(164, 50); 
    spr.print("ALL-TIME");
    spr.setTextColor(TOAST_COLOR, CARD_COLOR); 
    spr.setTextSize(2); 
    spr.setCursor(164, 60);
    spr.print(lt);
    ...
    spr.setTextColor(TFT_MAGENTA, CARD_COLOR); 
    spr.setTextSize(2); 
    spr.setCursor(86, 102); 
    spr.print(sb);

    drawCard(160, 88, 76, 38);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
    spr.setTextSize(1); 
    spr.setCursor(164, 92); 
    spr.print("RAVEN");
    spr.setTextColor(TEAL_COLOR, CARD_COLOR); 
    spr.setTextSize(2); 
    spr.setCursor(164, 102); 
    spr.print(sr); 
    ...
    if (sd_available) { 
        spr.print("SD: "); 
        spr.setTextColor(TEXT_COLOR, BG_COLOR); 
        spr.print("PCAP LOGGING ON"); 
    } else { 
        spr.setTextColor(ALERT_COLOR, BG_COLOR); 
        spr.print("NO SD CARD"); 
    }
...
void draw_locator_screen() {
    xSemaphoreTake(dataMutex, portMAX_DELAY);
    bool active = locator_active;
    bool has_est = locator_has_estimate;
    String target_mac = locator_target_mac;
    String target_name = locator_target_name;
    int samples = locator_sample_count;
    int peak = locator_peak_rssi;
    float dist = locator_est_distance;
    float brng = locator_bearing;
    float conf_r = locator_confidence_radius;
    xSemaphoreGive(dataMutex);
    ...
    if (!active) {
        if (last_cap_type == "None") {
            ...
            spr.setTextSize(2);
            spr.setCursor(4, 40); 
            spr.print("No target device.");
            
            spr.setCursor(4, 64); 
            spr.print("Detect first,");
            
            spr.setCursor(4, 88); 
            spr.print("then press 'l'");
        } else {
            drawCard(4, 22, DISP_W - 8, 20);
            spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
            spr.setTextSize(1);
            ...
            spr.setTextSize(1);
            spr.setCursor(4, 82); 
            spr.print("Press 'l' to start locating");
        }
    } else if (!has_est) {
        drawCard(4, 22, DISP_W - 8, 20);
        spr.setTextColor(TOAST_COLOR, CARD_COLOR); 
        ...
        spr.setTextColor(DIM_COLOR, BG_COLOR);
        spr.setCursor(4, 120); 
        spr.print("Drive around area to collect...");
    } else {
        ...
        spr.print(String((int)brng) + " "); 
        spr.print(cardinals[card_idx]);

        drawCard(rx + 2, 98, 116, 22);
        spr.setTextColor(ACCENT_COLOR, CARD_COLOR); 
        spr.setTextSize(1);
        spr.setCursor(rx + 6, 102); 
        spr.print("ACCURACY +/-");
        
        spr.setTextColor(TEAL_COLOR, CARD_COLOR);
        spr.setCursor(rx + 6, 112); 
        spr.print(String(conf_r, 0) + "m");
    ...
    if (active) {
        ...
        spr.setTextColor(TEAL_COLOR, HILIGHT_BG); 
        spr.setTextSize(1);
        spr.setCursor(4, DISP_H - 10);
...
void draw_boot_logo() {
    ...
    M5Cardputer.Display.print("DETECTOR v6.0");
}
...
void run_boot_sequence() {
    draw_boot_screen("Initializing Core Systems...", 10); delay(200);
    draw_boot_screen("Mounting Flash Storage...", 30);    delay(200);
    draw_boot_screen("Starting WiFi Promiscuous...", 50); delay(200);
    draw_boot_screen("Initializing BLE Scanner...", 70);  delay(200);
    draw_boot_screen("Booting GNSS Module...", 90);       delay(200);
    draw_boot_screen("ARMED AND SCANNING", 100);          delay(400);
}
...
void setup() {
    ...
    // ← ADD THIS: show reset reason on screen for 3 seconds
    esp_reset_reason_t reason = esp_reset_reason();
    spr.fillSprite(BG_COLOR);
    spr.setTextColor(WARN_COLOR, BG_COLOR);
    spr.setTextSize(1);
    spr.setCursor(4, 20);
    spr.print("RESET REASON: ");
    switch(reason) {
        case ESP_RST_POWERON:   spr.print("POWER ON"); break;
        case ESP_RST_SW:        spr.print("SOFTWARE"); break;
        case ESP_RST_PANIC:     spr.print("PANIC/CRASH"); break;
        case ESP_RST_INT_WDT:   spr.print("INT WATCHDOG"); break;
        case ESP_RST_TASK_WDT:  spr.print("TASK WATCHDOG"); break;
        case ESP_RST_WDT:       spr.print("WATCHDOG"); break;
        case ESP_RST_DEEPSLEEP: spr.print("DEEP SLEEP"); break;
        case ESP_RST_BROWNOUT:  spr.print("BROWNOUT"); break;  // ← key one
        case ESP_RST_SDIO:      spr.print("SDIO"); break;
        default:                spr.print(reason); break;
    }
    ...
    // Initial fill of the 15-sample median array
    int16_t init_v = M5Cardputer.Power.getBatteryVoltage();
    for(int i = 0; i < BATTERY_SAMPLE_SIZE; i++) {
        battery_buffer[i] = init_v;
    }
    
    WiFi.mode(WIFI_STA); 
    delay(50);
    
    int32_t start_v = get_median_voltage();
    ...
    // Physics Boot Trap
    if (end_v > 3900 || (end_v - start_v >= 15)) {
    ...
    if (mount_success) {
        ...
        if (!SD.exists(current_pcap_file.c_str())) {
            ...
            if (pfile) {
                ...
                pfile.write((const uint8_t*)pcap_header, 24);
                pfile.close();
            }
        }
    }
...
void loop() {
    ...
    // Exactly one update per loop for the moving median filter
    update_battery_metrics();
    ...
    if (M5Cardputer.BtnA.wasClicked()) {
        if (!stealth_mode) {
            ...
            transition_screen(next_screen, 1);
        }
    }
    ...
    if (millis() - last_sd_flush_check >= 500) {
        ...
        xSemaphoreTake(dataMutex, portMAX_DELAY);
        if (sd_write_buffer.size() >= MAX_LOG_BUFFER || (millis() - last_sd_flush > SD_FLUSH_INTERVAL && !sd_write_buffer.empty())) {
            should_flush = true;
        }
        xSemaphoreGive(dataMutex);
    ...
    if (!stealth_mode) {
        static unsigned long last_fast_anim = 0;
        static unsigned long last_slow_ui = 0;
        unsigned long now = millis();
        
        static uint8_t last_loop_led = 255;
        
        if (is_device_charging(get_median_voltage())) {
            float breathe = (exp(sin(now / 1000.0f * PI)) - 0.36787944f) / 2.35040238f;
            uint8_t led_val = (uint8_t)(breathe * 60.0f); 
            if (led_val != last_loop_led) {
                set_cardputer_led(0, led_val, 0);
                last_loop_led = led_val;
            }
        } else {
            if (last_loop_led != 0) {
                set_cardputer_led(0, 0, 0);
