    ...
    void onResult(NimBLEAdvertisedDevice* advertisedDevice) {
        ...
        if (advertisedDevice->haveManufacturerData()) {
            ...
            if (check_manufacturer_id(mfg)) {
                ...
                if (has_tn_serial(mfg)) strncat(methods, "tn_serial ", 63);
            }
        }
...
void draw_header_spr(int screen_num) {
    static const char* screen_names[] = {
        ...
        "LIVE FEED", "GPS", "PROXIMITY", "DEVICE INFO", "LOCATOR"
    };
    ...
    spr.setCursor(7, 5); spr.print(screen_names[screen_num]);

    uint16_t bcol = display_bat > 50 ? ACCENT_COLOR : (display_bat > 20 ? WARN_COLOR : ALERT_COLOR);
...
void draw_toast_spr() {
    ...
    spr.setTextColor(TFT_BLACK, TOAST_COLOR); spr.setTextSize(2);
...
void draw_scanner_screen() {
    ...
    // DROP SHADOW
    spr.fillEllipse(radar_cx + 2, radar_cy + 3, radar_r + 1, (radar_r + 1) * TILT, DIM2_COLOR);

    // FILLED DISC BASE
    spr.fillEllipse(radar_cx, radar_cy, radar_r, radar_r * TILT, CARD_COLOR);

    // THICK GLOWING RIM
    for (int t = 7; t >= 0; t--) {
        uint8_t intensity = (uint8_t)(30 + (7 - t) * 32);
        uint16_t rim_col = night_mode
            ? lgfx::color565(intensity, intensity / 6, intensity / 6)
            : lgfx::color565(0, intensity, intensity);
        spr.drawEllipse(radar_cx, radar_cy, radar_r - t, (radar_r - t) * TILT, rim_col);
    }

    // INNER RING
    spr.drawEllipse(radar_cx, radar_cy, inner_r, inner_r * TILT, GRID_COLOR);
    spr.drawEllipse(radar_cx, radar_cy, inner_r - 4, (inner_r - 4) * TILT, DIM2_COLOR);

    // PARTICLES
    static bool p_init = false;
    if (!p_init) {
        for (int i = 0; i < NUM_PARTICLES; i++) {
            noise_r[i]        = random(inner_r + 3, radar_r - 6);
            noise_a[i]        = random(0, 360) * 0.0174533f;
            noise_max_life[i] = random(200, 600);
            noise_life[i]     = random(0, noise_max_life[i]);
        }
        p_init = true;
    }
    for (int i = 0; i < NUM_PARTICLES; i++) {
        if (noise_life[i] <= 0) {
            ...
            uint16_t p_col = night_mode
                ...
                : ((i % 3 == 0) ? lgfx::color565(intensity, intensity, intensity)
                                : lgfx::color565(0, intensity, intensity));
            int px = radar_cx + (int)(noise_r[i] * cos(noise_a[i]));
            int py = radar_cy + (int)(noise_r[i] * sin(noise_a[i]) * TILT);
    ...
    for (int i = 1; i <= 28; i++) {
        float trail_angle = sweep_deg - (i * 0.045f);
        float ratio = (28.0f - i) / 28.0f;
        uint16_t fade_col = night_mode
            ? lgfx::color565((int)(220 * ratio), 0, 0)
            : lgfx::color565((int)(10 * ratio), (int)(238 * ratio), (int)(255 * ratio));
        for (int r_step = inner_r + 2; r_step < radar_r - 5; r_step += 3) {
            spr.drawPixel(
                radar_cx + (int)(r_step * cos(trail_angle)),
                radar_cy + (int)(r_step * sin(trail_angle) * TILT),
                fade_col);
        }
    }

    // SWEEP LINE
    spr.drawLine(radar_cx, radar_cy,
        radar_cx + (int)((radar_r - 6) * cos(sweep_deg)),
        radar_cy + (int)((radar_r - 6) * sin(sweep_deg) * TILT),
        HEADER_COLOR);

    // CROSSHAIRS
    float breathe   = sin(millis() / 500.0f);
    int   dynamic_r = radar_r - 7 + (int)(3 * breathe);
    uint16_t mod_col = (breathe > 0) ? HEADER_COLOR : DIM_COLOR;
    float cos_rot = cos(hud_rotation); float sin_rot = sin(hud_rotation);
    spr.drawLine(
        radar_cx - (int)(dynamic_r * cos_rot), radar_cy - (int)(dynamic_r * sin_rot * TILT),
        radar_cx + (int)(dynamic_r * cos_rot), radar_cy + (int)(dynamic_r * sin_rot * TILT), mod_col);
    spr.drawLine(
        radar_cx + (int)(dynamic_r * sin_rot), radar_cy - (int)(dynamic_r * cos_rot * TILT),
        radar_cx - (int)(dynamic_r * sin_rot), radar_cy + (int)(dynamic_r * cos_rot * TILT), mod_col);
    ...
    for (int i = 0; i < NUM_RADIAL_BANDS; i++) {
        ...
        if (radial_spikes[i] > 0) {
            int spike_len = (int)((radar_r - inner_r) * radial_spikes[i] * 0.65f);
            if (spike_len < 2) spike_len = 2;
            int base_x = radar_cx + (int)(radar_r * cos(a));
            int base_y = radar_cy + (int)(radar_r * sin(a) * TILT);
            spr.drawLine(base_x, base_y, base_x, base_y - spike_len, radial_colors[i]);
            spr.drawPixel(base_x, base_y - spike_len, TEXT_COLOR);
            spr.fillCircle(
                radar_cx + (int)((radar_r - 9) * cos(a)),
                radar_cy + (int)((radar_r - 9) * sin(a) * TILT),
                (int)(radial_spikes[i] * 2.5f), radial_colors[i]);
            radial_spikes[i] -= 0.0015f;
    ...
    if (!any_active) {
        ...
        spr.drawEllipse(radar_cx, radar_cy, pulse_r, (int)(pulse_r * TILT),
            (pulse_sin > 0) ? GRID_COLOR : BG_COLOR);
    }
    hud_rotation += 0.01f;
    spr.clearClipRect();
    ...
    if (last_cap_type != "None") {
        ...
        spr.setCursor(px + 2, 104); spr.print("-- no hits yet --");
    }
    draw_toast_spr();
}

// Screen 1: Combined Stats + Device Info
void draw_stats_screen() {
    ...
    // Version banner
    drawCard(4, 22, DISP_W - 8, 14);
    spr.setTextColor(HEADER_COLOR, CARD_COLOR); spr.setTextSize(1);
    spr.setCursor(8, 27); spr.print("FLOCK DETECTOR v6.5 [ADV]");

    // Session hit cards
    drawCard(4, 40, 72, 44);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(8, 44); spr.print("WIFI");
    spr.setTextColor(TOAST_COLOR, CARD_COLOR); spr.setTextSize(3); spr.setCursor(8, 54); spr.print(tw);

    drawCard(80, 40, 72, 44);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(84, 44); spr.print("BLE");
    spr.setTextColor(PURPLE_COLOR, CARD_COLOR); spr.setTextSize(3); spr.setCursor(84, 54); spr.print(tb);

    drawCard(156, 40, 80, 44);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(160, 44); spr.print("RAVEN");
    spr.setTextColor(TEAL_COLOR, CARD_COLOR); spr.setTextSize(3); spr.setCursor(160, 54); spr.print(tr);

    // Bottom row: session time / lifetime / all-time total
    drawCard(4, 88, 72, 38);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(8, 92); spr.print("SESSION");
    spr.setTextColor(TEXT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(8, 104);
    spr.print(format_time((millis() - session_start_time) / 1000));

    drawCard(80, 88, 72, 38);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(84, 92); spr.print("LIFETIME");
    spr.setTextColor(TEXT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(84, 104);
    spr.print(format_time(ls));

    drawCard(156, 88, 80, 38);
    spr.setTextColor(ACCENT_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(160, 92); spr.print("ALL-TIME");
    spr.setTextColor(TOAST_COLOR, CARD_COLOR); spr.setTextSize(2); spr.setCursor(160, 104); spr.print(lt);
...
void draw_live_log_screen() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        uint16_t txt_color = (proto_char == 'W') ? HEADER_COLOR : PURPLE_COLOR;
        spr.fillRect(0, y, DISP_W, 15, row_bg);
        if (is_hit) spr.fillRect(0, y, 3, 15, TOAST_COLOR);
        spr.setTextColor(txt_color, row_bg); spr.setTextSize(1);
        spr.setCursor(6, y + 3);
...
void draw_locator_screen() {
    ...
    xSemaphoreGive(dataMutex);
    spr.fillSprite(BG_COLOR); draw_header_spr(8);
    ...
    if (!active) {
        ...
        spr.setTextColor(TEXT_COLOR, BG_COLOR); spr.setCursor(4, 106);
        spr.print(samples); spr.print("/"); spr.print(LOC_MIN_SAMPLES_EST); spr.print("  Peak: "); spr.print(peak); spr.print("dBm");
        ...
        if (gps.course.isValid() && gps.speed.isValid() && gps.speed.mph() > 2.0) {
            rel_bearing = brng - gps.course.deg(); if (rel_bearing < 0) rel_bearing += 360.0;
        }
    ...
    if (active) {
        ...
        spr.setTextColor(TEAL_COLOR, HILIGHT_BG); spr.setTextSize(1); spr.setCursor(4, DISP_H - 10);
        spr.print(String(samples) + " pts | press 'l' to stop");
    }
}

// Screen 7: Device Info (standalone, minimal)
void draw_device_info_screen() {
    spr.fillSprite(BG_COLOR);
    draw_header_spr(7);
    drawCard(4, 22, DISP_W - 8, 20);
    spr.setTextColor(HEADER_COLOR, CARD_COLOR); spr.setTextSize(1); spr.setCursor(10, 27);
    spr.print("FLOCK DETECTOR v6.5 [ADV]");

    spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.setTextSize(1);
    spr.setCursor(4, 50); spr.print("Keys: </> screens  n=night  s=stealth");
    spr.setCursor(4, 62); spr.print("      m=mute  +/-=vol  l=locator");
    spr.setCursor(4, 74); spr.print("      t=cycle targets  0-9=jump");

    spr.drawLine(0, 86, DISP_W, 86, GRID_COLOR);

    spr.setCursor(4, 92); spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.print("WiFi CH: ");
    spr.setTextColor(TEXT_COLOR, BG_COLOR); spr.print(current_channel);
    spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.setCursor(80, 92); spr.print("Vol: ");
    spr.setTextColor(TEXT_COLOR, BG_COLOR); spr.print(current_volume);

    spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.setCursor(4, 104); spr.print("Uptime: ");
    spr.setTextColor(TEXT_COLOR, BG_COLOR); spr.print(format_time((millis() - session_start_time) / 1000));

    spr.drawLine(0, 116, DISP_W, 116, GRID_COLOR);
    spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.setTextSize(1); spr.setCursor(4, 120);
    if (sd_available) { spr.print("SD: "); spr.setTextColor(TEXT_COLOR, BG_COLOR); spr.print("PCAP LOGGING ON"); }
    else { spr.setTextColor(ALERT_COLOR, BG_COLOR); spr.print("NO SD CARD"); }

    spr.setCursor(140, 120); spr.setTextColor(ACCENT_COLOR, BG_COLOR); spr.print("GPS: ");
    spr.setTextColor(gps.location.isValid() ? ACCENT_COLOR : WARN_COLOR, BG_COLOR);
    spr.print(gps.location.isValid() ? "LOCK" : "SRCH");
...
void draw_current_screen() {
    switch (current_screen) {
        ...
        case 7: draw_device_info_screen();     break;
        case 8: draw_locator_screen();         break;
    }
}
...
void setup() {
    ...
    // BLE init FIRST — gets clean contiguous RAM before sprite
    ...
    // Sprite AFTER BLE — takes whatever contiguous block remains
    Serial.printf("Free heap AFTER BLE init: %d\n", ESP.getFreeHeap());
    Serial.printf("Largest free block AFTER BLE: %d\n", ESP.getMaxAllocHeap());
    Serial.printf("Sprite needs: %d bytes\n", DISP_W * DISP_H * 1);
    spr.setColorDepth(8);
    spr.createSprite(DISP_W, DISP_H);
    Serial.printf("Free heap AFTER sprite: %d\n", ESP.getFreeHeap());
...
void loop() {
    ...
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
        ...
        for (auto c : status.word) {
            ...
            else if (c == '0') { if (!stealth_mode) transition_screen(8, 1); }
            else if (c == 's') {
                stealth_mode = !stealth_mode;
                if (stealth_mode) { M5Cardputer.Display.setBrightness(0); }
                else { M5Cardputer.Display.setBrightness(200); draw_current_screen(); spr.pushSprite(0, 0); }
            }
            else if (c == 'l') {
                if (locator_active) { locator_stop(); }
                else if (last_cap_type != "None") { locator_start(last_cap_mac, last_cap_name); transition_screen(8, 1); }
            }
            else if (c == '\n' || c == '\r') {
                if (!stealth_mode) { int next = current_screen + 1; if (next >= NUM_SCREENS) next = 0; transition_screen(next, 1); }
            }
        }
        if (status.del && !stealth_mode) {
            int prev = current_screen - 1; if (prev < 0) prev = NUM_SCREENS - 1; transition_screen(prev, -1);
        }
    }

    if (millis() - last_time_save >= 1000) {
        xSemaphoreTake(dataMutex, portMAX_DELAY); lifetime_seconds++; xSemaphoreGive(dataMutex);
    ...
    if (millis() - last_sd_flush_check >= 500) {
        last_sd_flush_check = millis(); bool should_flush = false;
        xSemaphoreTake(dataMutex, portMAX_DELAY);
    ...
    if (!stealth_mode) {
        ...
        if (current_screen == 0) {
            ...
            if (now - last_slow_ui >= 100) { draw_current_screen(); spr.pushSprite(0, 0); last_slow_ui = now; }
        }
    }
    vTaskDelay(10 / portTICK_PERIOD_MS);
}
